//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:berth_api/api.dart';
import 'package:test/test.dart';

// tests for SecurityAuditLogInfo
void main() {
  // final instance = SecurityAuditLogInfo();

  group('test SecurityAuditLogInfo', () {
    // String actorIp
    test('to test the property `actorIp`', () async {
      // TODO
    });

    // String actorUserAgent
    test('to test the property `actorUserAgent`', () async {
      // TODO
    });

    // int actorUserId
    test('to test the property `actorUserId`', () async {
      // TODO
    });

    // String actorUsername
    test('to test the property `actorUsername`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // String eventCategory
    test('to test the property `eventCategory`', () async {
      // TODO
    });

    // String eventType
    test('to test the property `eventType`', () async {
      // TODO
    });

    // String failureReason
    test('to test the property `failureReason`', () async {
      // TODO
    });

    // int id
    test('to test the property `id`', () async {
      // TODO
    });

    // String metadata
    test('to test the property `metadata`', () async {
      // TODO
    });

    // int serverId
    test('to test the property `serverId`', () async {
      // TODO
    });

    // String sessionId
    test('to test the property `sessionId`', () async {
      // TODO
    });

    // String severity
    test('to test the property `severity`', () async {
      // TODO
    });

    // String stackName
    test('to test the property `stackName`', () async {
      // TODO
    });

    // bool success
    test('to test the property `success`', () async {
      // TODO
    });

    // int targetId
    test('to test the property `targetId`', () async {
      // TODO
    });

    // String targetName
    test('to test the property `targetName`', () async {
      // TODO
    });

    // String targetType
    test('to test the property `targetType`', () async {
      // TODO
    });

    // int targetUserId
    test('to test the property `targetUserId`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });


  });

}
